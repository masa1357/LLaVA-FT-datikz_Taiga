...
class GradePredictionDataset(Dataset):
    ...
    def _build_nested(self, df: pd.DataFrame) -> list[dict]:
        ...
        for uid, block in pivot.groupby(level=0):
            ...
            entry: dict[str, Any] = {
                ...
                "sentences": {},  # 各講義回の回答を格納する
            }
            for c in range(1, 16):
                key = f"L{c}"
                if (uid, c) in block.index:
                    entry["sentences"][key] = block.loc[(uid, c), qs_cols].to_dict()
                else:
                    entry["sentences"][key] = {q: self.fill_token for q in qs_cols}
    ...
    def trim_dataset(self) -> None:
        ...
        for sample in dataset:
            ...
            for c in range(1, 16):
                c_key = f"L{c}"
                if c_key not in sample["sentences"]:
                    continue
                for qn in self.q_filter:
                    q_key = f"Q{qn}"
                    ans = sample["sentences"][c_key].get(q_key, "")
                    # トークン数の取得
