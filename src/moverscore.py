...
device = 'cuda:0'
config = DistilBertConfig.from_pretrained(model_name, output_hidden_states=True, output_attentions=True,return_dict=False)
...
def bert_encode(model, x, attention_mask):
    ...
    x_ids  = torch.as_tensor(x, dtype=torch.long, device=device)
    attn_m = torch.as_tensor(attention_mask, dtype=torch.long, device=device)
    with torch.no_grad():
        output, x_encoded_layers, _ = model(input_ids = x_ids, attention_mask = attn_m)
